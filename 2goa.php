...
<section class="Goa-heading">
            <div class="Goa-heading-header">
                <h1>GOA - <i>India's Coastal Gem</i></h1>
            </div>
        </section>
<?php
...
$sql = "SELECT * FROM goa ORDER BY id DESC LIMIT 1";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>{$row['enddt']}</td>
                <td>{$row['members']}</td>
                <td>{$row['goaaccomodation']}</td>
                <td>{$row['transport']}</td>
                <td>{$row['goatourist']}</td>
                <td>{$row['placesToVisit']}</td>
                <td>{$row['activitiesToPerform']}</td>
            </tr>";
    }
